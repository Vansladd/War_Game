# ==============================================================
# $Id: export_BlueSQ.tcl,v 1.1.1.1 2011/10/04 10:54:25 xbourgui Exp $
#
# (C) 1999 Orbis Technology Ltd. All rights reserved.
# ==============================================================

namespace eval export_bluesq {

	namespace export create_export_file
...
proc populate_xgame_export_queries args {
    ...
    set xgaQry(set_xgame_ph_ex_file_created)  {
    	update tXGamePhExFile
	set    created = 'Y', created_at = CURRENT
    ...
    set xgaQry(get_xgame_ex_file_id) {
	select xgame_ex_file_id
	from   tXGameExFile
	where  filename = ?
    }
    ...
    # called by cron job
    # returns littlewoods games which have closed for entries 
    # since it was last called - 5 minutes before
    # only returns games if no file has been created since
    # the last of these games shut.

    set xgaQry(get_lw_games_closed_in_last_five_mins) {
	select max(g.shut_at) 
	from   tXGame g, tXGameExFile f 
	where  g.sort in ('PBUST3', 'PBUST4', 'SATPOOL', 'MONTHPOOL')
	and    g.shut_at <= current
	and    g.shut_at >= current - interval(5) minute to minute
	having max(g.shut_at) > max(f.cr_date)
    }

    set xgaQry(get_files) {
	select * 
	from   tXGameExFile
	where  filename > ?
    }

    set xgaQry(get_recent_files) {
    	select count(*)
	from   tXGamePhExFile
	where  created = 'Y'
	and    created_at >= current - interval(5) minute to minute 
    }
