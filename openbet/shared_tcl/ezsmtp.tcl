# $Id: ezsmtp.tcl,v 1.1 2011/10/04 12:23:19 xbourgui Exp $
# ezsmtp.tcl --
#
...
namespace eval ::ezsmtp {
	...
	set mail(timeout) 30000		;# connection to mail server times out at 30 seconds by default
...
proc ::ezsmtp::send {args} {
	...
	foreach v [list mailhost port from batchsize tclcharset mimecharset timeout] {
	...
	set aid [after $cur(timeout) {set ::ezsmtp::smtpConnected timeout}]
	set s [socket -async $cur(mailhost) $cur(port)]
	fileevent $s w {set ::ezsmtp::smtpConnected ok}
	vwait ::ezsmtp::smtpConnected
	...
	if {$::ezsmtp::smtpConnected == "timeout"} {
		catch [list close $s]
		error "connection to $cur(mailhost) on port $cur(port) timed out after $cur(timeout) ms"
	} else {
	...
	if {[info exists cur(body)]} {
		...
		foreach line [split $cur(body) \n] {
			...
			while {[string length $line] > 998} {
				...
				set line [string range $line 998 [expr \
...
proc ::ezsmtp::private_send_mail_headers {s var} {
	...
	# Multipart emails have no Content-Transfer-Encoding in their headers
...
proc ::ezsmtp::private_log {level msg} {
	...
	if {$mail(verbose) >= $level} {
		if {[string length $mail(logproc)]} {
			uplevel #0 [list $mail(logproc) $msg]
	} else {
			puts stdout "$msg"
			flush stdout
	}
